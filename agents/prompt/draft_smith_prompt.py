...
# Goal
读取具体的 `chapter_outline` (单章大纲) 和相关的 `character_data`，创作出一章约 3000-4000 字的正文。确保在生成新章节时，包含上一章的结尾内容，以增强章节之间的衔接感。
"""
...
## Step 1: 场景构建与承接 (Scene Setup & Transition)
开篇必须先承接上一章的结尾（`上一张结尾钩子内容：【{cliffhanger}】`）：将上一章的最后一段或最后一句以原文或改写的形式作为本章开头的第一段或首句，用以保持章节之间的连续性和叙事衔接。
在承接完成后，接着用不超过 100 字的段落描写当前环境，渲染出大纲要求的情绪基调（【 {emotional_tone}】）。如果上一章结尾已经明确交代了环境或动作，可将环境描写合并为一句以避免重复。
* *比如基调是“压抑”，就写乌云、冷风、压迫感；但首句应能自然衔接上一章的最后一句。*

## Step 2: 剧情扩写 (Expansion)
依次对大纲中的 `plot_points` 进行扩写。正文生成器应按以下优先级决定每个情节点的扩写长度：
1. 如果情节点为对象结构并包含 `suggested_expansion` 字段，则优先按照该值（以字为单位）扩写；
2. 否则，如果情节点包含 `sub_beats` 列表，则按情节点整体至少扩写为 600–800 字，并在 `sub_beats` 间合理分配；
3. 若情节点仅为简单字符串，则每个 `plot_point` 至少扩展为 600–800 字。
整体目标：使全章总字数接近 3000–4000 字（必要时可微调各情节点的扩写长度以满足章节总量要求）。
...
以大纲中的结尾钩子内容为基础，编写本章的最后一段。结尾钩子内容：【{cliffhanger}】
