...
def normalize_story_memory(value: Any) -> dict[str, Any]:
    ...
    for key in ("recent_chapter_summaries", "active_threads", "resolved_threads", "knowledge_fragments"):
        if not isinstance(memory.get(key), list):
            memory[key] = []
    if not isinstance(memory.get("roadmap_completion"), dict):
        memory["roadmap_completion"] = {}
...
def merge_lore_into_story_memory(
    ...
    *,
    ...
    _merge_roadmap_completion(memory, lore.get("roadmap_updates", {}), volume_num=volume_num)
...
def story_context_for_plot(story_memory: dict[str, Any] | None) -> str:
    memory = normalize_story_memory(story_memory)
    parts: list[str] = []
...
def story_context_for_draft(story_memory: dict[str, Any] | None, lore_records: list[dict[str, Any]] | None = None) -> str:
    ...
    recent = memory.get("recent_chapter_summaries", [])[-5:]
    if recent:
        summaries = [
            str(item.get("summary_text", "")).strip()
            for item in recent
            if isinstance(item, dict) and item.get("summary_text")
        ]
        if summaries:
            parts.append("最近五章事实摘要：\n" + "；".join(summaries))

    if lore_records:
        recent_lore = []
        for record in lore_records[-2:]:
            if not isinstance(record, dict):
                continue
            compact = _compact_lore_for_context(record)
            if compact:
                recent_lore.append(compact)
        if recent_lore:
            parts.append("最近两章 lore：\n" + "\n".join(f"- {line}" for line in recent_lore))

    return "\n\n".join(part for part in parts if part).strip()


def completed_roadmap_stage_indexes(story_memory: dict[str, Any] | None, volume_num: int) -> set[int]:
    memory = normalize_story_memory(story_memory)
    raw_values = memory.get("roadmap_completion", {}).get(str(volume_num), [])
    completed = set()
    if isinstance(raw_values, list):
        for value in raw_values:
            coerced = _coerce_int(value)
            if coerced is not None:
                completed.add(coerced)
    return completed
...
def _merge_roadmap_completion(memory: dict[str, Any], roadmap_updates: Any, *, volume_num: int | None) -> None:
    if volume_num is None or not isinstance(roadmap_updates, dict):
        return
    raw_indexes = roadmap_updates.get("completed_stage_indexes", [])
    if not isinstance(raw_indexes, list):
        return

    key = str(volume_num)
    completion = memory.get("roadmap_completion", {})
    existing = completion.get(key, [])
    completed = {
        value
        for value in (_coerce_int(item) for item in existing)
    ...
    for item in raw_indexes:
        ...
        if value is not None and value > 0:
            completed.add(value)
    completion[key] = sorted(completed)
    memory["roadmap_completion"] = completion


def _compact_lore_for_context(record: dict[str, Any]) -> str:
    parts = []
    chapter_id = record.get("chapter_id")
    if chapter_id:
        parts.append(f"第{chapter_id}章")
    summary = _clean_text(record.get("summary_text", ""))
    if summary:
        parts.append(summary)

    threads = record.get("plot_threads", {})
    if isinstance(threads, dict):
        opened = [
            item.get("description", "")
            for item in threads.get("opened", []) or []
            if isinstance(item, dict) and item.get("description")
        ]
        closed = [str(item) for item in threads.get("closed", []) or [] if str(item).strip()]
        if opened:
            parts.append("新线索：" + "；".join(opened[:3]))
        if closed:
            parts.append("解决线索：" + "；".join(closed[:3]))

    fragments = []
    for item in record.get("knowledge_fragments", []) or []:
        if isinstance(item, dict):
            topic = _clean_text(item.get("topic", ""))
            fact = _clean_text(item.get("fact", ""))
            if topic or fact:
                fragments.append(f"{topic}：{fact}".strip("："))
    if fragments:
        parts.append("新增设定：" + "；".join(fragments[:3]))

    status_changes = [str(item).strip() for item in record.get("character_status_changes", []) or [] if str(item).strip()]
    if status_changes:
        parts.append("状态变化：" + "；".join(status_changes[:3]))
